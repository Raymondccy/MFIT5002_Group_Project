...
type Medinfo struct {
	...
	Historys	[]HistoryItem	// 当前med的历史记录
}
...
type MedinfoChaincode struct {

}

func (t *MedinfoChaincode) Init(stub shim.ChaincodeStubInterface) peer.Response{
	fmt.Println(" ==== Init ====")

	return shim.Success(nil)
}

func (t *MedinfoChaincode) Invoke(stub shim.ChaincodeStubInterface) peer.Response{
	// 获取用户意图
	fun, args := stub.GetFunctionAndParameters()

	if fun == "addMed"{
		return t.addMed(stub, args)		// 添加信息
	}else if fun == "queryMedByCertNoAndName" {
		return t.queryMedByCertNoAndName(stub, args)		// 根据证书编号及姓名查询信息
	}else if fun == "queryMedInfoByEntityID" {
		return t.queryMedInfoByEntityID(stub, args)	// 根据身份证号码及姓名查询详情
	}else if fun == "updateMed" {
		return t.updateMed(stub, args)		// 根据证书编号更新信息
	}else if fun == "delMed"{
		return t.delMed(stub, args)	// 根据证书编号删除信息
	}
...
// 保存med
// args: Medinfo
func PutMed(stub shim.ChaincodeStubInterface, med Medinfo) ([]byte, bool) {
	...
	// 保存med状态
	err = stub.PutState(med.EntityID, b)
	if err != nil {
		return nil, false
	}
...
// 根据身份证号码查询信息状态
// args: entityID
func GetMedInfo(stub shim.ChaincodeStubInterface, entityID string) (Medinfo, bool)  {
	var med Medinfo
	// 根据身份证号码查询信息状态
...
// 添加信息
// args: medcationObject
// 身份证号为 key, Medinfo 为 value
func (t *MedinfoChaincode) addMed(stub shim.ChaincodeStubInterface, args []string) peer.Response {
	...
	// 查重: 身份证号码必须唯一
	_, exist := GetMedInfo(stub, med.EntityID)
	if exist {
...
// 根据证书编号及姓名查询信息
// args: CertNo, name
func (t *MedinfoChaincode) queryMedByCertNoAndName(stub shim.ChaincodeStubInterface, args []string) peer.Response {
...
// 根据身份证号码查询详情（溯源）
// args: entityID
func (t *MedinfoChaincode) queryMedInfoByEntityID(stub shim.ChaincodeStubInterface, args []string) peer.Response {
	if len(args) != 1 {
		return shim.Error("给定的参数个数不符合要求")
	}
	...
	err = json.Unmarshal(b, &med)
	if err != nil {
		return  shim.Error("反序列化Medinfo信息失败")
	}

	// 获取历史变更数据
	iterator, err := stub.GetHistoryForKey(med.EntityID)
	if err != nil {
	...
	for iterator.HasNext() {
		...
		if hisData.Value == nil {
			var empty Medinfo
			historyItem.Medinfo = empty
		}else {
			historyItem.Medinfo = hisMed
		}
...
// 根据身份证号更新信息
// args: medinfoObject
func (t *MedinfoChaincode) updateMed(stub shim.ChaincodeStubInterface, args []string) peer.Response {
	if len(args) != 2{
		return shim.Error("给定的参数个数不符合要求")
	}
	...
	// 根据身份证号码查询信息
	result, bl := GetMedInfo(stub, info.EntityID)
	if !bl{
...
// 根据身份证号删除信息（暂不提供）
// args: entityID
func (t *MedinfoChaincode) delMed(stub shim.ChaincodeStubInterface, args []string) peer.Response {
	if len(args) != 2{
		return shim.Error("给定的参数个数不符合要求")
	}
...
func main(){
	err := shim.Start(new(MedinfoChaincode))
	if err != nil{
		fmt.Printf("启动MedinfoChaincode时发生错误: %s", err)
	}
}
